...
function IPMenu(a, c, b, d) {
    var e = [];
    e[0] = '<a href="https://www.nic.ru/whois/?searchWord=' + a + '" target="_blank">' + c + "</a>";
...
function ajax_save_for_edit(a, c) {
    ...
    ShowLoading("");
    $.post(dle_root + "engine/ajax/controller.php?mod=editnews", b, function(a) {
        HideLoading("");
...
function ajax_prep_for_edit(a, c) {
    for (var b = 0, d = c_cache.length; b < d; b++)
        b in c_cache && (c_cache[b] || "" != c_cache[b]) && ajax_cancel_comm_edit(b);
    ShowLoading("");
...
function ajax_comm_edit(a, c) {
    for (var b = 0, d = c_cache.length; b < d; b++)
        b in c_cache && "" != c_cache[b] && ajax_cancel_comm_edit(b);
...
function doRate(a, c) {
    ...
    $.get(dle_root + "engine/ajax/controller.php?mod=rating", {
        ...
        if (a.success) {
            ...
            $("#likes-id-" + c).html(a.likes);
            $("#dislikes-id-" + c).html(a.dislikes)
        } else
...
function doCommentsRate(a, c) {
    ...
    $.get(dle_root + "engine/ajax/controller.php?mod=ratingcomments", {
        ...
        if (a.success) {
            ...
            $("#comments-likes-id-" + c).html(a.likes);
            $("#comments-dislikes-id-" + c).html(a.dislikes)
        } else
            a.error && DLEalert(a.errorinfo, dle_info)
    }, "json")
}
...
function DLESendPM(a) {
    ...
    c[dle_p_send] = function() {
        ...
        if ("" == e || "" == c || "" == a)
            return DLEalert(dle_req_field, dle_info),
            !1;
        k && (l = grecaptcha.getResponse(recaptcha_widget));
        h || (h = 0);
        g || (g = "");
        f || (f = "");
        ShowLoading("");
        $.post(dle_root + "engine/ajax/controller.php?mod=pm", {
            ...
            HideLoading("");
            a.success ? ($("#dlesendpmpopup").remove(),
            DLEalert(a.success, dle_info)) : a.error && DLEalert(a.error, dle_info)
        }, "json");
        return !1
    }
    ;
...
function dle_reply(a, c, b) {
    ...
    "1" == b && ($("#dlefastreplycomments").remove(),
    ...
    d[dle_p_send] = function() {
        ...
        $.post(dle_root + "engine/ajax/controller.php?mod=addcomments", {
            ...
            HideLoading("");
            $("#blind-animation" + a).remove();
            $("#comments-tree-item-" + a).length ? ($("#comments-tree-item-" + a).append(b),
            ...
            setTimeout(function() {
                $("#blind-animation" + a).show("blind", {}, 700)
            }, 600))) : $("#comment-id-" + a).length && ($("#comment-id-" + a).append(b),
            "error" != b && document.getElementById("blind-animation" + a) && ($("#dlereplypopup").remove(),
            $("html,body").stop().animate({
    ...
    $.get(dle_root + "engine/ajax/controller.php?mod=replycomments", {
        ...
        $("#dlefastreplycomments").html(c),
        $("html,body").stop().animate({
            scrollTop: $("#comment-id-" + a).offset().top + $("#comment-id-" + a).height() - 100
        }, 600),
        setTimeout(function() {
            $("#dlefastreplycomments").show("blind", {}, 700)
        }, 600)) : ($("body").append("<div id='dlereplypopup' title='" + dle_reply_title + "' style='display:none'></div>"),
        $("#dlereplypopup").html(c),
...
function CommentsPage(a, c, b) {
    ...
    $.get(dle_root + "engine/ajax/controller.php?mod=comments", {
        ...
        HideLoading("");
        isNaN(a) || isNaN(c) || ($("#dle-comm-link").off("click"),
...
function dle_fastreply(a) {
    ...
    $("#comments").froalaEditor("html.insert", b, !0)) : tinyMCE.execCommand("mceInsertContent", !1, b));
    setTimeout(function() {
        $("html,body").stop().animate({
...
function ShowOrHide(a) {
    ...
    3E3 < a && (a = 3E3);
    250 > a && (a = 250);
    "none" == c.css("display") ? (c.show("blind", {}, a),
    b && (b.src = dle_root + "templates/" + dle_skin + "/dleimages/spoiler-minus.gif")) : (2E3 < a && (a = 2E3),
    c.hide("blind", {}, a),
    b && (b.src = dle_root + "templates/" + dle_skin + "/dleimages/spoiler-plus.gif"))
}
...
function MenuNewsBuild(a, c) {
    ...
    allow_dle_delete_news && (b[2] = "<a onclick=\"sendNotice ('" + a + '\'); return false;" href="#">' + dle_notice + "</a>",
    b[3] = "<a onclick=\"dle_news_delete ('" + a + '\'); return false;" href="#">' + dle_del_news + "</a>");
    return b
}
...
function DLEprompt(a, c, b, d, e) {
    ...
    ;
    $("#dlepopup").remove();
    $("body").append("<div id='dlepopup' class='dle-promt' title='" + b + "' style='display:none'>" + a + "<br /><br /><input type='text' name='dle-promt-text' id='dle-promt-text' class='ui-widget-content ui-corner-all' style='width:97%;' value='" + c + "'/></div>");
    $("#dlepopup").dialog({
        autoOpen: !0,
        width: 500,
        resizable: !1,
...
function media_upload(a, c, b, d) {
    ...
    $("#mediaupload").dialog({
        ...
        open: function(f, k) {
            $("#mediaupload").html("<iframe name='mediauploadframe' id='mediauploadframe' width='100%' height='550' src='" + dle_root + "engine/ajax/controller.php?mod=upload&area=" + a + "&author=" + c + "&news_id=" + b + "&wysiwyg=" + d + "&skin=" + dle_skin + "&rndval=" + e + "' frameborder='0' marginwidth='0' marginheight='0' allowtransparency='true'></iframe>");
...
jQuery(function(a) {
    ...
    a(document).keydown(function(b) {
        if (13 == b.which && b.ctrlKey) {
            ...
            ;
            a("#dlepopup").remove();
            a("body").append("<div id='dlepopup' class='dle-promt' title='" + dle_orfo_title + "' style='display:none'><textarea name='dle-promt-text' id='dle-promt-text' class='ui-widget-content ui-corner-all' style='width:97%;height:80px;'></textarea><div id='orfom' style='display:none'>" + c + "</div></div>");
            a("#dlepopup").dialog({
                autoOpen: !0,
                width: 600,
                resizable: !1,
    ...
    setTimeout(function() {
        a("img[data-maxwidth]").each(function() {
            var b = a(this).width()
              , e = a(this).data("maxwidth");
            a(this)[0].naturalWidth && (b = a(this)[0].naturalWidth);
            b > e && (a(this).width(e),
            a(this).wrap('<a href="' + a(this).attr("src") + '" onclick="return hs.expand(this)"></a>'),
            "undefined" == typeof hs && 0 == c && (c = !0,
